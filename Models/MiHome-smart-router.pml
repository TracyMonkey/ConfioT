...
/******************** Mihome_smart_router *************************/
// Share（Client_A→ Client_B）in “MiHome app” using “member” role
inline Mihome_smart_router_SHARE(user_A, user_B, device_id){
    atomic{
        printf("'Mihome_smart_router': Share (user_%d → user_%d) in 'MiHome app' using 'member' role \n", user_A, user_B);
...
proctype ProcessHost(){
    ...
    bool COMPETE_Mihome_smart_router_SHARE = false;
        ...
        :: 
            atomic{
                if
                    :: (COMPETE_Philips_bridge_SHARE == false) ->
                        COMPETE_Philips_bridge_SHARE = true;
                        Mihome_smart_router_SHARE(host, guest, Devices[2].id);
                fi;
            }
        :: else -> break;
    od;
